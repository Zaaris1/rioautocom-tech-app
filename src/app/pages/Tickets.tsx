...
export default function TicketsPage() {
  ...
  const loadStores = async (netId?: string) => {
    try {
      setStores(await listStores(netId ? { network_id: netId } : undefined));
    } catch {
            ...
            <tbody>
              {tickets.map(t => (
                <tr key={t.id}>
                  ...
                  <td>
                    <span className={"badge " + (t.priority === "URGENTE" ? "danger" : "")}>
                      {t.priority}
                    </span>
                  </td>
                  <td>
                    <div style={{ fontWeight:650 }}>{t.local}</div>
                    <div className="small" style={{ maxWidth:360, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>
                      {t.problem}
                    </div>
                  </td>
                  <td style={{ textAlign:"right" }}>
                    <Link className="btn primary" to={`/tickets/${t.id}`}>Abrir</Link>
                  </td>
                </tr>
              ))}
            </tbody>
